...
export const QCard: React.FC<{ children: React.ReactNode; className?: string; title?: string; titleClassName?: string }> = ({ children, className = '', title, titleClassName = '' }) => {
  return (
    <div className={`bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 p-5 rounded-tl-2xl rounded-tr-sm rounded-br-xl rounded-bl-md shadow-sm hover:shadow-md transition-shadow duration-300 ${className}`}>
      {title && <h3 className={`text-lg font-bold text-stone-800 dark:text-stone-200 mb-4 border-b-2 border-stone-100 dark:border-stone-800 pb-2 inline-block transform -rotate-[0.5deg] ${titleClassName}`}>{title}</h3>}
      {children}
    </div>
  );
};
