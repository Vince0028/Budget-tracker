  ...
  <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div className="bg-white dark:bg-stone-900 p-8 rounded-tl-3xl rounded-br-[60px] max-w-md w-full shadow-2xl border-b-8 border-r-8 border-stone-800 dark:border-stone-100">
      ...
      <div className="prose dark:prose-invert text-sm text-stone-600 dark:text-stone-400 space-y-4 mb-8 font-medium">
        <p>1. <span className="font-bold">Device Only:</span> Your data lives in your browser. We don't see your bank statements or your midnight snack purchases.</p>
...
const App: React.FC = () => {
  ...
  const addTransaction = async (t: Transaction) => {
    ...
    if (user) {
      ...
      else setState(prev => ({ ...prev, transactions: [t, ...prev.transactions] }));
    }
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row bg-stone-50 dark:bg-stone-950 transition-colors duration-500`}>
      <aside className="w-full md:w-64 bg-stone-100 dark:bg-stone-900 border-r-2 border-stone-200 dark:border-stone-800 flex flex-col h-auto md:h-screen sticky top-0 z-40">
        <div className="p-8 pb-4">
          <h1 className="text-3xl font-black tracking-tighter text-stone-900 dark:text-stone-100 flex items-center gap-2">
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto h-screen bg-paper/10 dark:bg-stone-950">
        <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-700">
          ...
          {currentView === 'transactions' && <Transactions transactions={state.transactions} onAdd={addTransaction} onDelete={deleteTransaction} />}
